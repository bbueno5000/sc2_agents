...
import baselines.deepq.utils as utils
import dill
import numpy as np
import os
...
#     return U.BatchInput((16, 16), name=name)
    ...
    def make_obs_ph(name):
        return utils.PlaceholderTfInput(tf.placeholder(tf.float32, [None] + list((64, 64)), name=name))

    act_x, train_x, update_target_x, _ = deepq.build_train(
    ...
    act_y, _, update_target_y, _ = deepq.build_train(
        make_obs_ph=make_obs_ph,
        q_func=q_func,
        num_actions=num_actions,
        optimizer=tf.train.AdamOptimizer(learning_rate=lr),
    ...
    if prioritized_replay:
        ...
        beta_schedule_y = LinearSchedule(
            prioritized_replay_beta_iters,
            initial_p=prioritized_replay_beta0,
            final_p=1.0)
    else:
    ...
    with tempfile.TemporaryDirectory() as _:
        ...
        for t in range(max_timesteps):
            ...
            if done:
                ...
                # player_relative = obs[0].observation["screen"][_PLAYER_RELATIVE]
                player_relative = obs[0].observation.feature_screen.player_relative
                _ = (player_relative == _PLAYER_NEUTRAL).astype(int)
                player_y, player_x = (player_relative == _PLAYER_FRIENDLY).nonzero()
                player = [int(player_x.mean()), int(player_y.mean())]
                # Select all marines first
